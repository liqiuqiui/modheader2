  ...
  return (
    <div className="flex items-center gap-2 px-1 py-2">
      <button
        type="button"
        aria-label={
          enabled ? t("section.disableAll", { title }) : t("section.enableAll", { title })
        }
      ...
      <button
        type="button"
        aria-label={t("section.add", { title })}
        onClick={onAdd}
        className="rounded-lg p-1.5 text-slate-400 transition hover:bg-white hover:text-[var(--theme-color)]"
      >
      ...
      <DropdownMenu.Root>
        <DropdownMenu.Trigger asChild>
          <button
            type="button"
            aria-label={t("section.more", { title })}
            ...
            >
              <Switch
                ...
                onCheckedChange={(enabled) =>
                  onChange(
                    cookies.map((item) => (item.id === cookie.id ? { ...item, enabled } : item)),
                  )
                }
              ...
              <button
                ...
                onClick={() => onChange(cookies.filter((item) => item.id !== cookie.id))}
...
function FilterSection({
  ...
  const addFilter = (kind: FilterKind, seed?: Partial<FilterView>) => {
    ...
    if (kind === "urlPattern" || kind === "urlRegex") {
      ...
      if (mode === "exclude")
        onUpdate({ excludeUrlFilters: [...profile.excludeUrlFilters, filter] });
      ...
      onUpdate({
        tabFilters: [
          ...profile.tabFilters,
          createTabFilter({ tabId: activeTab, exclude: mode === "exclude" }),
        ],
      });
  ...
  const updateFilter = (filter: FilterView, patch: Partial<FilterView>) => {
    ...
    if (filter.kind === "urlPattern" || filter.kind === "urlRegex") {
      ...
      if (next.mode === filter.mode) {
        onUpdate(
          filter.mode === "include" ? { urlFilters: updated } : { excludeUrlFilters: updated },
        );
      } else {
  ...
  const setAllEnabled = () => {
    ...
    onUpdate({
      ...
      resourceFilters: profile.resourceFilters.map((filter) => ({
        ...filter,
        enabled: nextEnabled,
      })),
  ...
  return (
    <section>
      <div className="flex items-center gap-2 px-1 py-2">
        <button
          type="button"
          aria-label={enabled ? t("filter.disableAll") : t("filter.enableAll")}
          onClick={setAllEnabled}
          className={clsx(
            "flex h-4 w-4 items-center justify-center rounded border transition",
            enabled
              ? "border-[var(--theme-color)] bg-[var(--theme-color)] text-white"
        ...
        <button
          ...
          className={clsx(
            ...
            showAddPanel ? "bg-white text-[var(--theme-color)]" : "text-slate-400",
          )}
        >
          <Plus
            aria-hidden="true"
            className={clsx("h-4 w-4 transition", showAddPanel && "rotate-45")}
          />
        </button>
        <DropdownMenu.Root>
          <DropdownMenu.Trigger asChild>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3 rounded-xl bg-slate-50 p-3">
                ...
                {draftKind === "tab" && (
                  <div
                    ...
                    data-testid="current-tab-default"
                  >
                    <span className="font-semibold text-slate-700">
                      {t("filter.currentTabDefault")}
                    </span>
                    <span className="ml-1">
                      {activeTab?.title?.trim() || t("filter.noCurrentTab")}
                    </span>
                  </div>
                )}
            ...
            >
              ...
              <input
                ...
                onChange={(event) =>
                  onChange(
                    replacements.map((current) =>
                      current.id === item.id ? { ...current, value: event.target.value } : current,
                    ),
                  )
                }
              />
              <button
                ...
                onClick={() => onChange(replacements.filter((current) => current.id !== item.id))}
                className="rounded-lg p-2 text-slate-400 transition hover:bg-rose-50 hover:text-rose-600"
              >
                <X aria-hidden="true" className="h-4 w-4" />
              </button>
...
export default function App({ mode = "options" }: { mode?: "options" | "popup" } = {}) {
  ...
  const handlePause = async () => {
    await handleUpdateProfile({ paused: !profile.paused });
  };
    ...
    <ThemePortalProvider themeColor={profile.backgroundColor}>
      ...
      >
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-0.5">
              <button
                ...
                onClick={() => void undo()}
                className={iconButtonClass(history.past.length === 0)}
              >
              ...
              <button
                ...
                onClick={() => void redo()}
                className={iconButtonClass(history.future.length === 0)}
              >
              ...
              <DropdownMenu.Root>
                ...
                <DropdownMenu.Portal>
                  ...
                  >
                    <DropdownMenu.Item
                      className={menuItemClass}
                      onSelect={() => void i18n.changeLanguage("zh-CN")}
                    >
              ...
              <DropdownMenu.Root>
                ...
                <DropdownMenu.Portal>
                  <DropdownMenu.Content
                    align="end"
                    sideOffset={8}
                    onCloseAutoFocus={handleProfileMenuCloseAutoFocus}
                    className="z-[100] min-w-52 rounded-xl border border-slate-200 bg-white p-1.5 text-slate-800 shadow-xl"
                  >
                    <DropdownMenu.Item className={menuItemClass} onSelect={requestTitleRename}>
                    ...
                    <DropdownMenu.Item
                      className={menuItemClass}
                      onSelect={() => window.setTimeout(() => colorInputRef.current?.click(), 0)}
                    >
                      <Palette aria-hidden="true" className="h-4 w-4" /> {t("profile.changeColor")}
                    </DropdownMenu.Item>
                    <DropdownMenu.Separator className="my-1 h-px bg-slate-100" />
                    <DropdownMenu.Item className={menuItemClass} onSelect={exportProfile}>
                      <FileDown aria-hidden="true" className="h-4 w-4" /> {t("profile.exportFile")}
                    </DropdownMenu.Item>
                    <DropdownMenu.Item
                      className={menuItemClass}
                      onSelect={() => void copyProfile()}
                    >
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            >
              {mode !== "popup" && (
                <div className="mb-4 flex flex-wrap items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
                  <Switch
                    ...
                    onCheckedChange={(enabled) => void handleUpdateProfile({ enabled })}
              ...
              <div className={clsx(mode === "popup" ? "space-y-2.5" : "space-y-4")}>
                <HeaderSection
                ...
                {profile.urlReplacements.length > 0 && (
                  <RedirectSection
                    ...
                    searchQuery={searchQuery}
                    onChange={(urlReplacements) => void handleUpdateProfile({ urlReplacements })}
                  />
                )}
                <FilterSection
                  profile={profile}
                      ...
                      <DropdownMenu.Item
                        ...
                        onSelect={() => {
                          ...
                          void handleUpdateProfile({
                            respHeaders: [...profile.respHeaders, nextRule],
                          });
                        }}
                      >
        ...
        <input
          ...
          className="sr-only"
          onChange={(event) =>
            void handleUpdateProfile({ backgroundColor: event.target.value, textColor: "white" })
          }
        />
        {notice && (
          <div
