...
import { toPersistedState } from "../../../types/profile/profile-persistence";
...
describe("profile state storage", () => {
  ...
  it("round-trips canonical state with revision metadata", async () => {
    ...
    const persisted = {
      state: toPersistedState([profile], profile.id, false),
      revision: 4,
      sourceId: "client-a",
    };
  ...
  it("rejects invalid profile storage data", async () => {
    storageMocks.state.getValue.mockResolvedValue({ schemaVersion: 1 });

    await expect(readStoredProfileDocument()).rejects.toThrow("Invalid profile storage document");
  });
