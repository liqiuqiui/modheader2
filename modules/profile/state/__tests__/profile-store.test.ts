...
describe("profile store synchronization", () => {
  ...
  it("accepts the new revision stream after storage resets to an empty document", async () => {
    ...
    await profileStore.getState().initialize("zh-CN");
    expect(profileStore.getState().revision).toBe(100);

    mocks.emitStorageDocument(createEmptyProfileDocument());
    await vi.waitFor(() => expect(profileStore.getState().revision).toBe(1));
  ...
  it("resets the revision watermark even when storage is cleared during a pending command", async () => {
    ...
    const patch = profileStore.getState().patchProfile(original.id, { title: pending.title });
    mocks.emitStorageDocument(createEmptyProfileDocument());
  ...
  it("retries initialization when the storage reset changes while initialization is in flight", async () => {
    ...
    const { profileStore } = await import("../profile-store");
    const initialization = profileStore.getState().initialize("zh-CN");
    ...
    await vi.waitFor(() => expect(profileStore.getState().selectedProfileId).toBe(fresh.id));
  });
  ...
  it("rolls back an optimistic edit when command and storage reconciliation both fail", async () => {
    ...
    await expect(
      profileStore.getState().patchProfile(original.id, { title: "Not persisted" }),
    ).resolves.toBe(false);
    ...
    expect(profileStore.getState().past).toEqual([]);
  });
});
